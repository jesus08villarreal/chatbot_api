...
def select_products(message: str, products: List[Dict]) -> List[Dict]:
        try:
            response = client.chat.completions.create(
                ...
                response_format={ "type": "json_object" },
                messages=[
                    {"role": "system", "content": "You are a helpful assistant. that helps to select a product based on the information provided that is a list and a message with the product information. and you will return a list of products in a json object."},
